...
type CompetitorRow = {
  // Discriminator — the venue's own row uses `is_self: true` and skips trend / detail.
  is_self:        boolean;
  rank:           number | null;     // 1-indexed within the bucket; null for own row in `other`
  name:           string;
  // Header-level chips
  ...
  // Tap-to-expand detail panel. Null for own venue's self-row.
...
| Self-row (in `direct`) | constructed from the own `venues` row, slotted by its rank position; `is_self=true`, `detail=null`, `trend=null` |
